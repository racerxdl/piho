...
describe("canonical compiler", () => {
  ...
  test("matches the complete golden image and decodes every table", async () => {
    ...
    expect(decodeGraphImage(image)).toEqual({
      ...
      routes: [
        { id: 1, flowId: 1, inputId: 1, ownerDevice: 1, edge: "rising", actionIds: [1, 3] },
        { id: 2, flowId: 2, inputId: 2, ownerDevice: 2, edge: "changed", actionIds: [2] },
      ],
      actions: [
        {
          id: 2,
  ...
  test("reports manifest, counts, offsets, sizes, and checksum", async () => {
    ...
    expect(report).toEqual({
      ...
      sections: {
        ...
        routes: { offset: 136, bytes: 24, records: 2 },
        actionReferences: { offset: 160, bytes: 6, records: 3 },
        actions: { offset: 166, bytes: 48, records: 3 },
      },
      devices: [
        {
          id: 1,
